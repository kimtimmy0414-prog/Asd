...
foreach ($files as $f) {
    $path = DATA_DIR . '/' . $f;
    if (!file_exists($path)) {
        $init = ($f === 'schedules.json' ? '[]' : '[]');
        file_put_contents($path, $init);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login') {
        ...
        echo "<script>alert('아이디 또는 비밀번호가 틀립니다.'); history.back();</script>"; exit;
    }

    if ($action === 'write_post' && current_user()) {
        $posts = json_read('posts.json');
        $new_id = empty($posts) ? 1 : max(array_column($posts,'id')) + 1;
        $posts[] = ['id'=>$new_id, 'title'=>trim($_POST['title']), 'content'=>trim($_POST['content']), 'author_id'=>current_user()['id'], 'author_name'=>current_user()['username'], 'date'=>date('Y-m-d H:i:s'), 'views'=>0, 'created'=>time()];
...
<head>
    ...
    <style>
        ...
        .hero { background: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('https://via.placeholder.com/1920x400/aa0000/ffffff?text=LOTTE+GIANTS'); background-size:cover; color:white; }
...
<div class="hero py-5 text-center">
    <div class="container">
        <h1 class="display-3 fw-bold">We are Giants!</h1>
        <p class="lead">부산 갈매기 팬클럽 - 롯데클럽 ⚾</p>
    </div>
</div>
...
<div class="container mt-4">
    <div class="row">
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    ...
                    <form method="post">
                        ...
                        <div class="row g-2">
                            ...
                            <div class="col-md-3"><input type="text" name="reason" class="form-control" placeholder="지급 사유 (선택)"></div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="post">
                        ...
                        <textarea name="content" class="form-control mb-2" rows="3" placeholder="내용" required></textarea>
            ...
            <h4>최신 게시물</h4>
            <?php foreach (array_slice(array_reverse($posts), 0, 8) as $post): ?>
            <div class="card mb-3 post-card">
                <div class="card-body">
                    <h5><a href="?view=<?= $post['id'] ?>" class="text-decoration-none"><?= htmlspecialchars($post['title']) ?></a></h5>
                    ...
                    <div class="mt-2 small text-muted">댓글 <?= count($post['comments']) ?>개</div>
                    <?php endif; ?>
                    <?php if ($user): ?>
                    <form method="post" class="mt-3 input-group">
                        <input type="hidden" name="action" value="add_comment">
                        <input type="hidden" name="post_id" value="<?= $post['id'] ?>">
                        <input type="text" name="comment" class="form-control" placeholder="댓글 입력" required>
        ...
        <div class="col-lg-4">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <hr>
                    <h6>인기 게시물 (댓글 많은 순)</h6>
                    <?php 
                    $popular = $posts;
                    usort($popular, fn($a,$b) => count($b['comments'] ?? []) <=> count($a['comments'] ?? []));
                    foreach (array_slice($popular, 0, 5) as $p): ?>
                        <div><a href="?view=<?= $p['id'] ?>" class="text-decoration-none"><?= htmlspecialchars($p['title']) ?></a> (<?= count($p['comments'] ?? []) ?>)</div>
                    <?php endforeach; ?>
                    ...
                    <?php 
                    $view_rank = $posts;
                    usort($view_rank, fn($a,$b) => ($b['views'] ?? 0) <=> ($a['views'] ?? 0));
                    foreach (array_slice($view_rank, 0, 5) as $p): ?>
                        <div><a href="?view=<?= $p['id'] ?>" class="text-decoration-none"><?= htmlspecialchars($p['title']) ?></a> (<?= $p['views'] ?? 0 ?>)</div>
                    <?php endforeach; ?>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body small">
                    ...
                    <?php else: 
                        ...
                        foreach (array_slice($schedules, 0, 6) as $s): ?>
                            <div><?= $s['date'] ?> <?= $s['time'] ?> <?= $s['is_home'] ? 'vs '.$s['opponent'].' (홈)' : '@ '.$s['opponent'] ?> (<?= $s['stadium'] ?>)</div>
                        <?php endforeach; 
                    endif; ?>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    회원 수 : <b><?= count($users) ?></b>명<br>
                    총 게시물 : <b><?= count($posts) ?></b>개
                </div>
            </div>
        </div>
    </div>
